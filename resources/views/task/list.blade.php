...
@foreach ($list as $task)
        <tr>
            ...
            <td><form style="margin: 0px;" action="{{ route('complete', ['task_id' => $task->id]) }}" method="post"> @csrf <button onclick='return confirm("このタスクを「完了」にします。よろしいですか?");' >完了</button></form></td>
@endforeach
        </table>
        <!--ページネーション-->
        {{--{{ $list->links() }} --}}
